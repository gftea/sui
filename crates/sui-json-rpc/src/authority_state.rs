...
use sui_json_rpc_types::{
    Checkpoint, Coin as SuiCoin, DevInspectResults, DryRunTransactionBlockResponse, EventFilter,
    SuiEvent, SuiObjectDataFilter, TransactionFilter,
};
...
pub trait State: Send + Sync {
    ...
    ) -> StateReadResult<(VerifiedTransaction, TransactionEffects)>;
    ...
    fn multi_get_executed_transactions(
        &self,
        digests: &[TransactionDigest],
    ) -> StateReadResult<Vec<Option<VerifiedTransaction>>>;

    fn multi_get_executed_effects(
        &self,
        digests: &[TransactionDigest],
    ) -> StateReadResult<Vec<Option<TransactionEffects>>>;
    ...
    ) -> StateReadResult<Vec<Option<TransactionEvents>>>;
    ...
    async fn get_transaction_block(
        &self,
        digest: TransactionDigest,
    ) -> StateReadResult<VerifiedTransaction>;

    fn get_executed_effects(
        &self,
        digest: TransactionDigest,
    ) -> StateReadResult<TransactionEffects>;
    ...
    ) -> StateReadResult<TransactionEvents>;
...
impl State for AuthorityState {
    ...
    async fn get_executed_transaction_and_effects(
        ...
        Ok(self.get_executed_transaction_and_effects(digest).await?)
    }
    ...
    fn get_transactions(
        ...
        Ok(self.get_transactions(filter, cursor, limit, reverse)?)
    }
    fn query_events(
        &self,
        query: EventFilter,
        ...
        Ok(self.query_events(query, cursor, limit, descending)?)
    }
    fn get_dynamic_field_object_id(
        &self,
        owner: ObjectID,
    ...
    fn multi_get_executed_transactions(
        ...
        Ok(self.multi_get_executed_transactions(digests)?)
    }
    ...
    fn multi_get_executed_effects(
        ...
        Ok(self.multi_get_executed_effects(digests)?)
    }
    ...
    fn multi_get_events(
        ...
        Ok(self.multi_get_events(digests)?)
    }
    ...
    async fn get_transaction_block(
        ...
        Ok(self.get_transaction_block(digest).await?)
    }
    ...
    fn get_executed_effects(
        ...
        Ok(self.get_executed_effects(digest)?)
    }
    ...
    fn get_transaction_events(
        ...
        Ok(self.get_transaction_events(digest)?)
    }
    ...
    fn get_checkpoints(
        ...
        Ok(self.get_checkpoints(cursor, limit, descending_order)?)
    }
...
#[async_trait]
impl<S: ?Sized + State> ObjectProvider for Arc<S> {
...
/// Also implements `From` trait to convert into `RpcError` if this is indeed the final error state.
#[derive(Debug, Error)]
...
impl From<StateReadError> for RpcError {
    fn from(e: StateReadError) -> Self {
        match e {
            StateReadError::Client(_) => RpcError::Call(CallError::InvalidParams(e.into())),
            _ => {
                let error_object = ErrorObject::owned(-32603, e.to_string(), None::<()>);
