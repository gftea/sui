...
impl AuthorityStore {
            ...
            .map(|kind| {
                match kind {
                    InputObjectKind::SharedMoveObject { id, initial_shared_version: _, mutable } => {
                        ...
                        (InputKey::Versioned{ id: *id, version: *version}, lock_mode)
                    }
                    ...
                    InputObjectKind::ImmOrOwnedMoveObject(objref) => (InputKey::Versioned {id: objref.0, version: objref.1}, LockMode::Default),
                }
            })
            .collect()
    }
    ...
    pub fn multi_input_objects_available(
        ...
        for ((idx, input_key), obj_wrapper_opt) in keys_with_version.iter().zip(
            self.perpetual_tables
                .objects
                .multi_get(
                    keys_with_version
    ...
    async fn update_objects_and_locks(
        ...
        let possible_to_receive = transaction
            ...
            .receiving_objects()
            .unwrap_or(vec![]);
        // Use this to get the actual set of objects that have been receivied -- any received
        // object will show up as a write or delete.
        let received_objects: Vec<_> = possible_to_receive
            .into_iter()
            .flat_map(
                |obj_ref| match (written.get(&obj_ref.0), deleted.get(&obj_ref.0)) {
                    (Some(_), _) | (_, Some(_)) => Some(obj_ref),
                    _ => None,
                },
            )
            .collect();
...
impl ChildObjectResolver for AuthorityStore {
    ...
    fn receive_object_at_version(
        ...
        // Invalid access -- treat as the object does not exist.
        if recv_object.owner != Owner::AddressOwner((*owner).into()) {
            return Ok(None);
        }

        // If we've already received the object at the version, then we say that it doesn't exist.
...
impl InputKey {
    ...
    pub fn version(&self) -> Option<SequenceNumber> {
        match self {
            InputKey::Versioned { version, .. } => Some(*version),
